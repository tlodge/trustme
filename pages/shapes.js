...
export default function Home(props) {
  ...
  useEffect(()=>{
    if (ended){
       ...
       if (answeredAll){
         const nd = nextDimension[dimension];
    ...
    <div className={styles.questiontext} dangerouslySetInnerHTML={{__html:questionText}}></div>
      ...
      <div style={{display:"flex", flexDirection:"row"}}>
          <div className={styles.feedbackcontainer}>
            <ThreePointFeedback selected={dimension == "d1" ? question: null} chaptercomplete={complete} answers={answers.d1} previous={previous} labels={true} clicked={()=>{/*_setDimension("d1");setQuestion("q1")*/}}/>
            <div className={styles.feedbacktext} style={{opacity: dimension == "d1" ? 1.0: 0.5}}>Knowledge</div>
          </div>
          <div className={styles.feedbackcontainer}>
            <FourPointFeedback  selected={dimension == "d2" ? question: null} chaptercomplete={complete} answers={answers.d2}  previous={previous} labels={true} clicked={()=>{/*_setDimension("d2");setQuestion("q1")*/}}/>
            <div className={styles.feedbacktext} style={{opacity: dimension == "d2" ? 1.0: 0.5}}>Choice</div>
          </div>
          <div className={styles.feedbackcontainer}>
            <FivePointFeedback  selected={dimension == "d3" ? question: null} chaptercomplete={complete} answers={answers.d3} previous={previous} labels={true}  clicked={()=>{/*_setDimension("d3");setQuestion("q1")*/}}/>
            <div className={styles.feedbacktext} style={{opacity: dimension == "d3" ? 1.0: 0.5}}>Risk</div>
          </div>
      </div>
      <div style={{display:"flex", minHeight:130, justifyContent:"center"}}>
          {complete && renderNavigation()}
      </div>
